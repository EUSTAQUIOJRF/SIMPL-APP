...
// Moedas mostradas na barra rolante — o dólar fica fixo/destacado fora da rolagem.
// Bandeiras como emoji (não imagem remota) de propósito: emoji renderiza na hora,
// sem depender de carregar nada da internet — é isso que elimina o "soluço" no letreiro.
      ...
      <View style={styles.faixaRolagem}>
        ...
        >
          {[...MOEDAS_LETREIRO, ...MOEDAS_LETREIRO].map((item, i) => (
            <View key={i} style={styles.itemCotacaoBarra}>
...
export default function SearchScreen({ navigation }) {
  ...
  // Busca a taxa de câmbio do país de embarque escolhido no modal de orçamento
  useEffect(() => {
  ...
  const valorConvertido =
    valorUsd && !isNaN(valorUsd) && taxaOrcamento
      ? (parseFloat(valorUsd) * taxaOrcamento.taxaUsd).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : null;
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: "#0f172a" }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      >
        <View style={styles.header}>
          <View style={styles.headerEsquerda}>
            ...
            <TouchableOpacity style={[styles.iconButton, { marginLeft: 8 }]} onPress={() => navigation.navigate("Historico", { idioma })}>
          ...
          <View style={styles.seletorIdiomas}>
            {["pt", "en", "es"].map((cod) => (
              <TouchableOpacity key={cod} style={[styles.btnBandeira, idioma === cod && styles.btnBandeiraAtivo]} onPress={() => setIdioma(cod)}>
                <Text style={styles.textoBandeira}>{{ pt: "🇧🇷", en: "🇺🇸", es: "🇪🇸" }[cod]}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>

        <Image source={require("../../logosemfundo2.png")} style={styles.logoImage} resizeMode="contain" />

        {cotacoes && <LetreiroCotacoes cotacoes={cotacoes} onPress={() => navigation.navigate("Conversor", { idioma })} />}

        <Text style={styles.titulo}>{t.titulo}</Text>
        <Text style={styles.subtitulo}>{t.subtitulo}</Text>

        <View style={styles.containerBotoesTopo}>
          <TouchableOpacity style={styles.botaoSurpresa} onPress={() => setModalSurpresaVisivel(true)}>
            <Text style={styles.botaoSurpresaTexto}>{t.btnSurpresa}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.botaoOrcamento} onPress={() => setModalOrcamentoVisivel(true)}>
        ...
        <View style={styles.formGroup}>
          ...
          <TextInput style={styles.input} placeholder={t.placeholderCidade} placeholderTextColor="#64748b" value={cidade} onChangeText={setCidade} />
        </View>

        <View style={styles.formGroup}>
          <Text style={styles.label}>{t.labelPaisEmbarque} <Text style={styles.obrigatorio}>*</Text></Text>
          <TextInput style={styles.input} placeholder={t.placeholderPaisEmbarque} placeholderTextColor="#64748b" value={paisEmbarque} onChangeText={setPaisEmbarque} />
        </View>

        <View style={styles.row}>
          <View style={[styles.formGroup, { flex: 1, marginRight: 8 }]}>
            <Text style={styles.label}>{t.labelDataIda} <Text style={styles.obrigatorio}>*</Text></Text>
            <TextInput style={styles.input} placeholder="DD/MM/AAAA" placeholderTextColor="#64748b" value={dataIda} onChangeText={setDataIda} />
          </View>
          <View style={[styles.formGroup, { flex: 1, marginLeft: 8 }]}>
            <Text style={styles.label}>{t.labelDataVolta}</Text>
            <TextInput style={styles.input} placeholder="DD/MM/AAAA" placeholderTextColor="#64748b" value={dataVolta} onChangeText={setDataVolta} />
          </View>
        </View>

        <View style={styles.formGroup}>
          <Text style={styles.label}>{t.labelPessoas}</Text>
          <TextInput style={styles.input} keyboardType="numeric" placeholder="2" placeholderTextColor="#64748b" value={pessoas} onChangeText={setPessoas} />
        </View>

        <View style={styles.formGroup}>
          <Text style={styles.label}>{t.labelRegiaoDesejada}</Text>
          <TextInput style={styles.input} placeholder={t.placeholderRegiaoDesejada} placeholderTextColor="#64748b" value={regiaoDesejada} onChangeText={setRegiaoDesejada} />
        </View>

        <TouchableOpacity style={styles.botaoBuscar} onPress={handleBuscar} disabled={carregando}>
          {carregando ? <ActivityIndicator color="#ffffff" /> : <Text style={styles.textoBotaoBuscar}>{t.botaoBuscar}</Text>}
        </TouchableOpacity>
        ...
        <View style={styles.secaoParcerias}>
          ...
          <View style={styles.gridParcerias}>
            <TouchableOpacity style={styles.cardParceria} onPress={() => Linking.openURL("https://ektatraveling.tpx.gr/OeHONXnD")}>
            ...
            <TouchableOpacity style={styles.cardParceria} onPress={() => Linking.openURL("https://airalo.tpx.gr/vSwWU9MG")}>
            ...
            <TouchableOpacity style={styles.cardParceria} onPress={() => Linking.openURL("https://kiwi.tpx.gr/dqV74yCf")}>
              <Text style={styles.iconeParceria}>✈️</Text>
              <Text style={styles.tituloParceria}>{t.passagens}</Text>
              <Text style={styles.subtituloParceria}>Kiwi.com</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.cardParceria} onPress={() => Linking.openURL("https://klook.tpx.gr/c4GAtMTM")}>
              <Text style={styles.iconeParceria}>🎟️</Text>
        ...
        <View style={styles.secaoNoticias}>
          ...
          {carregandoNoticias ? (
            ...
            noticias.map((item, i) => (
              <TouchableOpacity key={i} style={styles.cardNoticia} onPress={() => item.link && Linking.openURL(item.link)}>
      ...
      <Modal visible={modalSurpresaVisivel} animationType="slide" transparent onRequestClose={() => setModalSurpresaVisivel(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={{ maxHeight: 300 }}>
              <View style={styles.containerPerfis}>
                {PERFIS_SURPRESA.map((perfil) => {
                  ...
                  return (
                    <TouchableOpacity key={perfil} style={[styles.chipPerfil, selecionado && styles.chipPerfilSelecionado]} onPress={() => togglePerfil(perfil)}>
                      <Text style={[styles.textoChipPerfil, selecionado && styles.textoChipPerfilSelecionado]}>{perfil}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
            ...
            <TouchableOpacity style={styles.botaoFecharModal} onPress={() => setModalSurpresaVisivel(false)}>
      ...
      <Modal visible={modalOrcamentoVisivel} animationType="slide" transparent onRequestClose={() => setModalOrcamentoVisivel(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <TouchableOpacity style={styles.botaoFecharTopo} onPress={() => { setModalOrcamentoVisivel(false); setResultadoOrcamento(null); }}>
            ...
            <TextInput style={styles.input} placeholder="Brasil" placeholderTextColor="#64748b" value={paisEmbarqueOrcamento} onChangeText={setPaisEmbarqueOrcamento} />
            ...
            <View style={styles.linhaValorConversao}>
              ...
              <View style={styles.campoConvertido}>
                <Text style={styles.labelConvertido}>{taxaOrcamento ? taxaOrcamento.codigo : "..."}</Text>
            ...
            <TextInput style={styles.input} placeholder="Outubro" placeholderTextColor="#64748b" value={mesViagem} onChangeText={setMesViagem} />

            <TouchableOpacity style={styles.botaoBuscarOrcamento} onPress={handleBuscarPorOrcamento} disabled={carregandoOrcamento}>
              {carregandoOrcamento ? <ActivityIndicator color="#ffffff" /> : <Text style={styles.textoBotaoBuscar}>{t.btnBuscarOrcamento}</Text>}
            </TouchableOpacity>

            {resultadoOrcamento && (
              <ScrollView style={{ maxHeight: 200, marginTop: 12 }}>
                {resultadoOrcamento.destinos?.map((item, i) => (
                  <TouchableOpacity key={i} style={styles.cardDestino} onPress={() => abrirResultadoOrcamento(item)}>
                    <Text style={styles.nomeDestinoCard}>{item.cidade}, {item.pais}</Text>
                    <Text style={styles.detalheCusto}>Est. Total: US$ {item.custoTotalEstimadoUsd}</Text>
            ...
            <TouchableOpacity style={styles.botaoFecharModal} onPress={() => { setModalOrcamentoVisivel(false); setResultadoOrcamento(null); }}>
              <Text style={styles.textoFecharModal}>{t.btnVoltaModal}</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  botaoSair: { paddingHorizontal: 12, paddingVertical: 8, backgroundColor: "#1e293b", borderRadius: 20, borderWidth: 1, borderColor: "#334155" },
  botaoSairTexto: { color: "#94a3b8", fontSize: 13, fontWeight: "600" },
  seletorIdiomas: { flexDirection: "row", alignItems: "center", backgroundColor: "#1e293b", borderRadius: 16, paddingHorizontal: 4, paddingVertical: 2, borderWidth: 1, borderColor: "#334155" },
  ...
  logoImage: { width: 200, height: 110, alignSelf: "center", marginBottom: 10 },
  iconButton: { padding: 8, backgroundColor: "#1e293b", borderRadius: 20, borderWidth: 1, borderColor: "#334155" },
  iconText: { fontSize: 15 },
  containerBarraCotacoes: { flexDirection: "row", alignItems: "center", backgroundColor: "#1e293b", borderRadius: 12, marginBottom: 18, paddingVertical: 8, borderWidth: 1, borderColor: "#334155", overflow: "hidden" },
  badgeBaseUsd: { backgroundColor: "#0f172a", paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, marginLeft: 8, marginRight: 8, borderWidth: 1, borderColor: "#38bdf8" },
  textUsdBase: { color: "#4ade80", fontSize: 12, fontWeight: "bold" },
  faixaRolagem: { flex: 1, overflow: "hidden" },
  itemCotacaoBarra: { flexDirection: "row", alignItems: "center", backgroundColor: "#0f172a", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginRight: 8, borderWidth: 1, borderColor: "#334155" },
  ...
  containerBotoesTopo: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20, gap: 10 },
  botaoSurpresa: { flex: 1, paddingVertical: 10, borderRadius: 20, backgroundColor: "#0a1120", borderColor: "#1e293b", borderWidth: 1, alignItems: "center" },
  botaoSurpresaTexto: { color: "#38bdf8", fontSize: 13, fontWeight: "700" },
  botaoOrcamento: { flex: 1, paddingVertical: 10, borderRadius: 20, backgroundColor: "#0a1120", borderColor: "#1e293b", borderWidth: 1, alignItems: "center" },
  ...
  input: { backgroundColor: "#1e293b", color: "#ffffff", borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, borderWidth: 1, borderColor: "#334155" },
  row: { flexDirection: "row" },
  linhaValorConversao: { flexDirection: "row", gap: 8, alignItems: "center" },
  campoConvertido: { backgroundColor: "#0f172a", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#38bdf8", minWidth: 90, alignItems: "center" },
  ...
  botaoBuscar: { backgroundColor: "#2563eb", paddingVertical: 14, borderRadius: 8, alignItems: "center", marginTop: 10 },
  ...
  cardParceria: { width: "48%", backgroundColor: "#1e293b", borderRadius: 10, padding: 12, alignItems: "center", borderWidth: 1, borderColor: "#334155", marginBottom: 8 },
  ...
  botaoBuscarOrcamento: { backgroundColor: "#16a34a", paddingVertical: 12, borderRadius: 8, alignItems: "center", marginTop: 10 },
  textoBotaoBuscar: { color: "#ffffff", fontSize: 15, fontWeight: "bold" },
  cardDicaDia: { backgroundColor: "#1e293b", borderColor: "#0284c7", borderWidth: 1, borderRadius: 12, padding: 14, marginTop: 20 },
  tituloDicaDia: { color: "#38bdf8", fontWeight: "bold", fontSize: 14, marginBottom: 4 },
  textoDicaDia: { color: "#cbd5e1", fontSize: 13, lineHeight: 18 },
  secaoNoticias: { marginTop: 24, marginBottom: 20 },
  tituloSecaoNoticias: { color: "#ffffff", fontSize: 16, fontWeight: "bold", marginBottom: 12 },
  cardNoticia: { backgroundColor: "#1e293b", borderRadius: 10, padding: 12, marginBottom: 10, borderWidth: 1, borderColor: "#334155" },
  ...
  modalTitulo: { color: "#ffffff", fontSize: 18, fontWeight: "bold", marginBottom: 16, textAlign: "center" },
  containerPerfis: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  chipPerfil: { backgroundColor: "#0f172a", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16, borderWidth: 1, borderColor: "#334155" },
  ...
  botaoConfirmarModal: { backgroundColor: "#2563eb", paddingVertical: 12, borderRadius: 8, alignItems: "center", marginTop: 16 },
  textoConfirmarModal: { color: "#ffffff", fontWeight: "bold" },
  botaoFecharModal: { marginTop: 14, alignItems: "center", paddingVertical: 10, backgroundColor: "#0f172a", borderRadius: 8, borderWidth: 1, borderColor: "#334155" },
  textoFecharModal: { color: "#cbd5e1", fontSize: 13, fontWeight: "600" },
  cardDestino: { backgroundColor: "#0f172a", padding: 12, borderRadius: 8, marginBottom: 8, borderWidth: 1, borderColor: "#334155" },
  ...
  avisoLegal: { color: "#94a3b8", fontSize: 11, fontStyle: "italic", marginTop: 8, textAlign: "center" },
});